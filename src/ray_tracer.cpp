...
{
  // Allocate arrays
  sample_pos.allocate(im_res, im_res, 4, im_max_steps);
  sample_dir.allocate(im_res, im_res, 4, im_max_steps);
  ...
  for (int m = 0; m < im_res; m++)
    for (int l = 0; l < im_res; l++)
    {
      ...
      k[0] = std::fmax(temp_q / temp_a, temp_c / temp_q);
