...
void RadiationIntegrator::CalculateSimulationCoefficients()
{
  ...
  if (first_time)
  {
    j_i.Allocate(image_resolution, image_resolution, image_steps);
    alpha_i.Allocate(image_resolution, image_resolution, image_steps);
    j_i.Zero();
    alpha_i.Zero();
    if (image_polarization)
    {
      j_q.Allocate(image_resolution, image_resolution, image_steps);
      j_v.Allocate(image_resolution, image_resolution, image_steps);
      alpha_q.Allocate(image_resolution, image_resolution, image_steps);
      alpha_v.Allocate(image_resolution, image_resolution, image_steps);
      rho_q.Allocate(image_resolution, image_resolution, image_steps);
      rho_v.Allocate(image_resolution, image_resolution, image_steps);
      j_q.Zero();
      j_v.Zero();
      alpha_q.Zero();
      alpha_v.Zero();
    ...
    for (int m = 0; m < image_resolution; m++)
      for (int l = 0; l < image_resolution; l++)
      {
        ...
        for (int n = 0; n < num_steps; n++)
        {
          ...
          double uu1_sim = sample_uu1(m,l,n);
          double uu2_sim = sample_uu2(m,l,n);
          double uu3_sim = sample_uu3(m,l,n);
          double bb1_sim = sample_bb1(m,l,n);
          double bb2_sim = sample_bb2(m,l,n);
          double bb3_sim = sample_bb3(m,l,n);
          ...
          j_i(m,l,n) = coefficient_j * exp_neg_xx_1_3 * coefficient_1 * sin_theta_b * factor_i
              * factor_i / nu_2_cgs;
          if (image_polarization)
          {
            ...
            j_q(m,l,n) = -coefficient_j * exp_neg_xx_1_3 * coefficient_1 * sin_theta_b * factor_q
                * factor_q / nu_2_cgs;
            j_v(m,l,n) = coefficient_j * exp_neg_xx_1_3 * coefficient_2
          ...
          alpha_i(m,l,n) = nu_3_cgs * j_i(m,l,n) / b_nu_cgs;
          if (image_polarization)
          {
            alpha_q(m,l,n) = nu_3_cgs * j_q(m,l,n) / b_nu_cgs;
            alpha_v(m,l,n) = nu_3_cgs * j_v(m,l,n) / b_nu_cgs;
          }

          // Account for numerical issues later arising from absorptivities being too small
          if (1.0 / (alpha_i(m,l,n) * alpha_i(m,l,n)) == std::numeric_limits<double>::infinity())
          {
            alpha_i(m,l,n) = 0.0;
            if (image_polarization)
            {
          ...
          if (image_polarization)
          {
            ...
            rho_q(m,l,n) = coefficient_rho * nu_c_cgs * sin2_theta_b / nu_2_cgs * factor_fm
