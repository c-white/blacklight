// Blacklight ray tracer - coordinates, metric components, and tetrad components
...
// Function for calculating Christoffel connection components in simulation coordinates
// Inputs:
//   x1, x2, x3: coordinates
// Output:
...
void RayTracer::CoordinateConnection(double x1, double x2, double x3, Array<double> &connection)
{
  // Account for simulation metric
  switch (simulation_coord)
  {
    ...
    case Coordinates::sph_ks:
    {
      ...
      // Calculate covariant metric component derivatives
      double dgcov[4][4][4] = {};
      double temp_a = 2.0 * bh_m * (sigma - 2.0 * r2) / sigma2;
      double temp_b = -bh_a * sth2 * temp_a;
      double temp_c = 4.0 * bh_m * a2 * r * sth * cth / sigma2;
      double temp_d = -4.0 * bh_m * bh_a * r * (r2 + a2) * sth * cth / sigma2;
      double temp_e = temp_d - 2.0 * bh_a * sth * cth;
      dgcov[1][0][0] = temp_a;
      dgcov[1][0][1] = temp_a;
      dgcov[1][0][3] = temp_b;
      dgcov[1][1][0] = temp_a;
      ...
      dgcov[1][3][0] = temp_b;
      dgcov[1][3][1] = temp_b;
      dgcov[1][3][3] = (2.0 * r - bh_a * temp_b) * sth2;
      dgcov[2][0][0] = temp_c;
      dgcov[2][0][1] = temp_c;
      dgcov[2][0][3] = temp_d;
      dgcov[2][1][0] = temp_c;
      dgcov[2][1][1] = temp_c;
      dgcov[2][1][3] = temp_e;
      dgcov[2][2][2] = -2.0 * a2 * sth * cth;
      dgcov[2][3][0] = temp_d;
      dgcov[2][3][1] = temp_e;
      dgcov[2][3][3] =
          -bh_a * sth2 * temp_d + 2.0 * sth * cth * (r2 + a2 + 2.0 * bh_m * a2 * r * sth2 / sigma);
    ...
    case Coordinates::cart_ks:
    {
      ...
      dgcov[0][0][0] = +(df_dx * l0 * l0 + f * dl0_dx * l0 + f * l0 * dl0_dx);
      dgcov[0][0][1] = -(df_dx * l0 * l1 + f * dl0_dx * l1 + f * l0 * dl1_dx);
      dgcov[0][0][2] = -(df_dx * l0 * l2 + f * dl0_dx * l2 + f * l0 * dl2_dx);
      dgcov[0][0][3] = -(df_dx * l0 * l3 + f * dl0_dx * l3 + f * l0 * dl3_dx);
      dgcov[0][1][0] = -(df_dx * l1 * l0 + f * dl1_dx * l0 + f * l1 * dl0_dx);
      dgcov[0][1][1] = +(df_dx * l1 * l1 + f * dl1_dx * l1 + f * l1 * dl1_dx);
      dgcov[0][1][2] = +(df_dx * l1 * l2 + f * dl1_dx * l2 + f * l1 * dl2_dx);
      dgcov[0][1][3] = +(df_dx * l1 * l3 + f * dl1_dx * l3 + f * l1 * dl3_dx);
      dgcov[0][2][0] = -(df_dx * l2 * l0 + f * dl2_dx * l0 + f * l2 * dl0_dx);
      dgcov[0][2][1] = +(df_dx * l2 * l1 + f * dl2_dx * l1 + f * l2 * dl1_dx);
      dgcov[0][2][2] = +(df_dx * l2 * l2 + f * dl2_dx * l2 + f * l2 * dl2_dx);
      dgcov[0][2][3] = +(df_dx * l2 * l3 + f * dl2_dx * l3 + f * l2 * dl3_dx);
      dgcov[0][3][0] = -(df_dx * l3 * l0 + f * dl3_dx * l0 + f * l3 * dl0_dx);
      dgcov[0][3][1] = +(df_dx * l3 * l1 + f * dl3_dx * l1 + f * l3 * dl1_dx);
      dgcov[0][3][2] = +(df_dx * l3 * l2 + f * dl3_dx * l2 + f * l3 * dl2_dx);
      dgcov[0][3][3] = +(df_dx * l3 * l3 + f * dl3_dx * l3 + f * l3 * dl3_dx);

      // Calculate covariant metric component y-derivatives
      dgcov[1][0][0] = +(df_dy * l0 * l0 + f * dl0_dy * l0 + f * l0 * dl0_dy);
      dgcov[1][0][1] = -(df_dy * l0 * l1 + f * dl0_dy * l1 + f * l0 * dl1_dy);
      dgcov[1][0][2] = -(df_dy * l0 * l2 + f * dl0_dy * l2 + f * l0 * dl2_dy);
      dgcov[1][0][3] = -(df_dy * l0 * l3 + f * dl0_dy * l3 + f * l0 * dl3_dy);
      dgcov[1][1][0] = -(df_dy * l1 * l0 + f * dl1_dy * l0 + f * l1 * dl0_dy);
      dgcov[1][1][1] = +(df_dy * l1 * l1 + f * dl1_dy * l1 + f * l1 * dl1_dy);
      dgcov[1][1][2] = +(df_dy * l1 * l2 + f * dl1_dy * l2 + f * l1 * dl2_dy);
      dgcov[1][1][3] = +(df_dy * l1 * l3 + f * dl1_dy * l3 + f * l1 * dl3_dy);
      dgcov[1][2][0] = -(df_dy * l2 * l0 + f * dl2_dy * l0 + f * l2 * dl0_dy);
      dgcov[1][2][1] = +(df_dy * l2 * l1 + f * dl2_dy * l1 + f * l2 * dl1_dy);
      dgcov[1][2][2] = +(df_dy * l2 * l2 + f * dl2_dy * l2 + f * l2 * dl2_dy);
      dgcov[1][2][3] = +(df_dy * l2 * l3 + f * dl2_dy * l3 + f * l2 * dl3_dy);
      dgcov[1][3][0] = -(df_dy * l3 * l0 + f * dl3_dy * l0 + f * l3 * dl0_dy);
      dgcov[1][3][1] = +(df_dy * l3 * l1 + f * dl3_dy * l1 + f * l3 * dl1_dy);
      dgcov[1][3][2] = +(df_dy * l3 * l2 + f * dl3_dy * l2 + f * l3 * dl2_dy);
      dgcov[1][3][3] = +(df_dy * l3 * l3 + f * dl3_dy * l3 + f * l3 * dl3_dy);

      // Calculate covariant metric component z-derivatives
      dgcov[2][0][0] = +(df_dz * l0 * l0 + f * dl0_dz * l0 + f * l0 * dl0_dz);
      dgcov[2][0][1] = -(df_dz * l0 * l1 + f * dl0_dz * l1 + f * l0 * dl1_dz);
      dgcov[2][0][2] = -(df_dz * l0 * l2 + f * dl0_dz * l2 + f * l0 * dl2_dz);
      dgcov[2][0][3] = -(df_dz * l0 * l3 + f * dl0_dz * l3 + f * l0 * dl3_dz);
      dgcov[2][1][0] = -(df_dz * l1 * l0 + f * dl1_dz * l0 + f * l1 * dl0_dz);
      dgcov[2][1][1] = +(df_dz * l1 * l1 + f * dl1_dz * l1 + f * l1 * dl1_dz);
      dgcov[2][1][2] = +(df_dz * l1 * l2 + f * dl1_dz * l2 + f * l1 * dl2_dz);
      dgcov[2][1][3] = +(df_dz * l1 * l3 + f * dl1_dz * l3 + f * l1 * dl3_dz);
      dgcov[2][2][0] = -(df_dz * l2 * l0 + f * dl2_dz * l0 + f * l2 * dl0_dz);
      dgcov[2][2][1] = +(df_dz * l2 * l1 + f * dl2_dz * l1 + f * l2 * dl1_dz);
      dgcov[2][2][2] = +(df_dz * l2 * l2 + f * dl2_dz * l2 + f * l2 * dl2_dz);
      dgcov[2][2][3] = +(df_dz * l2 * l3 + f * dl2_dz * l3 + f * l2 * dl3_dz);
      dgcov[2][3][0] = -(df_dz * l3 * l0 + f * dl3_dz * l0 + f * l3 * dl0_dz);
      dgcov[2][3][1] = +(df_dz * l3 * l1 + f * dl3_dz * l1 + f * l3 * dl1_dz);
      dgcov[2][3][2] = +(df_dz * l3 * l2 + f * dl3_dz * l2 + f * l3 * dl2_dz);
      dgcov[2][3][3] = +(df_dz * l3 * l3 + f * dl3_dz * l3 + f * l3 * dl3_dz);
...
// Inputs:
//   x, y, z: geodesic coordinates
// Output:
//   jacobian: components set
// Notes:
//   Assumes jacobian is allocated to be 4*4.
//   Jacobian contains dx_geodesic^mu/dx_coordinate^nu with indices (mu, nu).
void RayTracer::GeodesicCoordinateJacobian(double x, double y, double z, Array<double> &jacobian)
{
  // Account for simulation metric
  switch (simulation_coord)
  {
...
// Function for calculating orthonormal tetrad components in simulation coordinates
// Inputs:
...
void RayTracer::Tetrad(const double ucon[4], const double ucov[4], const double kcon[4],
