...
// Outputs: (none)
// Notes:
//   Assumes camera_pos[adaptive_level] and camera_dir[adaptive_level] have been set except for the
//       time components of the latter.
//   Initializes time components of camera_dir[adaptive_level].
//   Lowers all components of camera_dir[adaptive_level].
...
void GeodesicIntegrator::InitializeGeodesics()
{
  ...
  #pragma omp parallel
  {
    ...
    for (int m = 0; m < num_pix; m++)
    {
      // Extract position
      double x[4];
      x[0] = camera_pos[adaptive_level](m,0);
      x[1] = camera_pos[adaptive_level](m,1);
      x[2] = camera_pos[adaptive_level](m,2);
      x[3] = camera_pos[adaptive_level](m,3);

      // Extract spatial components of momentum
      double p[4];
      p[1] = camera_dir[adaptive_level](m,1);
      p[2] = camera_dir[adaptive_level](m,2);
      p[3] = camera_dir[adaptive_level](m,3);
